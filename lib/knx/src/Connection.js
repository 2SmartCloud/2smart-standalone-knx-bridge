...
FSM.prototype.read = function(grpaddr, dpt,  callback) {
  if(typeof dpt === 'function') {
    callback = dpt;
    dpt = undefined;
  }
  if (typeof callback == 'function') {
    ...
    var binding = function(src, data) {
        ...
        // fire the callback
        if (dpt) data = DPTLib.fromBuffer(data, DPTLib.resolve(dpt));
    ...
    var timeout = setTimeout(function() {
      conn.off(responseEvent, binding);
      clearTimeout(timeout);
      callback(new Error(`No response(${responseEvent}) timeout.`));
    }, 3000);
  }
  var serviceType = this.useTunneling ?
    KnxConstants.SERVICE_TYPE.TUNNELING_REQUEST :
...
FSM.prototype.AddHPAI = function(datagram) {
  datagram.hpai = {
    protocol_type: 1, // UDP
    tunnel_endpoint: (this.localEndpoint.address || '0.0.0.0') + ":" + (this.localEndpoint.receivePort || this.socket.address().port)
    //tunnel_endpoint: '192.168.2.25:3671'
